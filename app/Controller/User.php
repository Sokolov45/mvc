...
class User extends AbstractController
{
    ...
    делать так, что loginAction будет работать и как для самого активного действия (авторизации) если передан верно логин
    и пароль, а также для отображения формы (если не передано имя). Так типо будет удобно передавать ошибку. То есть
    один и тот же метод - мы просто в шаблон прокинем в 2 случаях ( if (!$user).. и  if (UserModel::getPass...) нашу ошибку
    ...
    назад на форму, но надо передать сообщение, что вы ввели неверно и типо между 2 разными экшенами сложно пере-
...
//        Нам нужно отрендерить шаблон регистрации
    ...
    public function registerAction()   //экшен для "зарегистрировать пользователя"
    {
//        if ($this->user) {
//            $this->redirect("/blog/index");
//        }
    ...
    public function profileAction() //научимся получать пользователя из базы
    {
        return $this->view->render('User/profile.phtml', [
            'user' => UserModel::getById($this->user->getId()), /*Здесь мы получаем пользователя и
             одновременно создаём объект c полученными из базы данными*/
            "Anton" => 'Sokol'
        ]);
    }
